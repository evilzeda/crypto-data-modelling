{{ config(
    ...
    unique_key=['week_id', 'token_id']
) }}

with vols as (
  select
    format_date('%G-%V', date(event_ts_utc)) as week_id,
    token_id,
...
market as (
  select week_id, sum(total_volume_usd) as market_volume_usd
  from vols
  group by 1
),
shares as (
